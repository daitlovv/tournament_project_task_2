...
void setup_round() {
    mutex_lock(&arena.arena_mutex);

    if (arena.finished) {
        mutex_unlock(&arena.arena_mutex);
        return;
    }
...
void* fighter_thread(void* arg) {
    ...
    while (1) {
        ...
        if (arena.fighters[fighter_id].has_rival) {
            ...
            do {
                ...
                if (winner_move == my_move) {
                    ...
                    mutex_lock(&arena.arena_mutex);
                     ...
                     arena.fighters[fighter_id].active &&
                     arena.fighters[rival_id].active);
            ...
            pthread_cond_wait(&arena.round_cond, &arena.arena_mutex);
        }
...
void print_usage(const char* prog_name) {
    ...
    printf("  %s -f config.txt -o results.txt\n", prog_name);
}
...
int main(int argc, char *argv[]) {
    ...
    // Открытие файла для вывода, если указано
    ...
    print_output("------ Турнир 'Камень-Ножницы-Бумага' (многопоточная версия) ------\n");
    print_output("Количество участников: %d\n", fighter_count);
    print_output("Используемые примитивы: мьютексы и условные переменные\n");
